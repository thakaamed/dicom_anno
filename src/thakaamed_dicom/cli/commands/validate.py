...
def validate(preset: str | None, config: Path | None) -> None:
    ...
    try:
        ...
        table.add_row("Description", preset_config.description[:60] + "..." if len(preset_config.description) > 60 else preset_config.description)
        ...
        table.add_row("Profile Options", ", ".join(profile_options) if profile_options else "None (Basic Profile)")
        ...
        console.print()
        console.print("[dim]Check the YAML syntax and ensure all required fields are present.[/dim]")
        raise click.ClickException(str(e)) from e
