...
class PDFReportBuilder:
    ...
    def _add_page_decorations(self, canvas_obj, doc):
        ...
        canvas_obj.drawRightString(
            page_width - 0.75 * inch, page_height - 0.28 * inch, "DICOM Anonymization Report"
        )
        ...
        canvas_obj.drawCentredString(
            page_width / 2, 0.6 * inch, "Vision 2030 Healthcare Transformation"
        )
        ...
        canvas_obj.setFont("Helvetica", 7)
        canvas_obj.drawCentredString(
            page_width / 2, 0.12 * inch, "https://thakaamed.ai | contact@thakaamed.com"
        )
    ...
    def _add_header(self, story, report_data: ReportData):
        ...
        story.append(Paragraph("De-identification Audit Trail", self.styles["ReportSubtitle"]))
    ...
    def _add_preset_config(self, story, report_data: ReportData):
        ...
        config_data = [
            ...
            [
                "Description",
                report_data.preset_description[:80] + "..."
                if len(report_data.preset_description) > 80
                else report_data.preset_description,
            ],
        ...
        legend_data = [
            ...
            [
                "C",
                "Clean",
                "Replace with values of similar meaning that don't identify the patient",
            ],
        ...
        if report_data.tag_rules_applied:
            story.append(
                Paragraph(
                    f"<font color='#2D1B4E'><b>📋 Complete Tag Rules Applied ({len(report_data.tag_rules_applied)} rules)</b></font>",
                    self.styles["Normal"],
                )
            )
            story.append(Spacer(1, 0.1 * inch))
